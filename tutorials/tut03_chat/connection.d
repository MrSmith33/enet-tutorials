...
abstract class Connection
{
	...
	ubyte[] createPacket(P)(auto ref P packet)
	{
		...
		version(debug_packets) writefln("creating packet %s with id %s", P.stringof, packetId!P);
		version(debug_packets) writefln("with fields {%s}:%s", packet.tupleof, numEncodableMembers!(packet));
		
		size = encodeCbor(bufferTemp[], packetId!P);
		
		version(debug_packets) writef("size %s, bufferTemp[0]%02x", size, bufferTemp[0]);
	...
	void onPacketReceived(ref ENetEvent event)
	{
		try
		{
			...
			version(debug_packets) writefln("%s:%s received len %s | text %s | hex %(%02x%)",
